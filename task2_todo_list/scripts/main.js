...
function addToDo(task_input, date_input, time_input, status) {
  let task = {
    id: getRandomId(),
  ...
  todos.forEach((todo) => {
    todos_list_body.innerHTML += `
            <tr class="todo-item" data-id="${todo.id}">
                ...
                <td>${todo.dueTime || "No Time selected"}</td>
                <td>${todo.status}</td>
                <td>
...
function filterTodos(status) {
  ...
  const hideDrop = document.getElementById("drop_hide");
  setTimeout(() => {
    hideDrop.classList.add("hide");
  }, 2000);
